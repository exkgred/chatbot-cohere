...
export function resolveIngestUrl() {
  const configured = cleanEnv(process.env.LOGS_INGEST_URL);
...
export function resolveIngestSecret() {
  const configured = cleanEnv(process.env.LOGS_INGEST_SECRET || process.env.INGEST_SECRET);
  if (configured) return configured;
  if (process.env.VERCEL) return DEV_INGEST_SECRET;
  return '';
}

function postIngest(url, secret, payload) {
...
export function shipConversationLog(payload) {
  ...
  if (!secret) console.warn('ingest warning: LOGS_INGEST_SECRET ausente');

  return postIngest(url, secret, payload)
    .then(async (response) => {
      if (response.status === 401 && secret !== DEV_INGEST_SECRET && process.env.VERCEL) {
        console.warn('ingest 401, tentando secret padrão');
        return postIngest(url, DEV_INGEST_SECRET, payload);
      }
      return response;
    })
    .then(async (response) => {
      ...
      try {
        ...
        if (loki && loki.status !== 'ok' && loki.status !== 'queued') {
